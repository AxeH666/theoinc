...
interface CreateBatchProps {
  onCancel: () => void;
  onSave: (data: { name: string; rows: BatchRow[] }) => void;
  isViewMode?: boolean;
  batchId?: string;
  status?: string;
  role: Role;
...
export const CreateBatch: React.FC<CreateBatchProps> = ({ 
  ...
  const [batchName] = useState(batchId || '');
      ...
      <div className={`fixed bottom-0 ${isSidebarCollapsed ? 'left-20' : 'left-64'} right-0 h-12 bg-slate-900 flex items-center justify-between px-6 z-20 transition-all duration-300`}>
        ...
        {!isReadOnly && (
          <div className="flex items-center gap-3">
            ...
            <Button
              onClick={() => onSave({ name: batchName, rows })}
              size="sm"
              className="rounded-none bg-blue-600 h-8 px-8 font-black uppercase tracking-widest border-none"
            >
              Finalize Sheet
            </Button>
          </div>
